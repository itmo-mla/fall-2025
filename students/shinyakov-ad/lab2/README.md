...
**Оптимальное значение:** `k = 1` или `k = 2` (оба дают LOO = 0.9831).
...
|---------------------------|----------|
| Parzen-KNN                | **1.000** |
| sklearn KNN (uniform)     | 0.963    |
| sklearn KNN (distance)    | 0.963    |
...
- После **CNN**:
  - CNN selected: **23** из 178 (очень агрессивное сжатие)
  - KNN on CNN prototypes acc: **0.978**
  - Вывод: сильная компрессия привела к относительно значительной потере качества.
- После **ENN**:
  - ENN selected: **175** из 178
  - Parzen KNN on ENN prototypes acc: **0.983**
  - Вывод: ENN удалил шум и не уменьшил качество; Parzen-KNN устойчив к такой очистке.
...
- LOO-подбор показал, что малые k (1–2) оптимальны для определения \(h\) в данном наборе.  
- CNN слишком агрессивен для этого набора: сильное сжатие привело к потере качества.  
- ENN — щадящий метод очистки: удалил немного «шумовых» объектов и сохранил/улучшил качество.  
- Для практических задач рекомендуется использовать Parzen-KNN совместно с ENN при необходимости убрать шум и снизить вычислительную нагрузку умеренно.
