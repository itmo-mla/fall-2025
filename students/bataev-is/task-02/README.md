...
$$
w_i = K\left(\frac{d_i}{h(x)}\right), \quad \hat{y}(x)=\arg\max_{c}\sum_{i \in N_k(x)} w_i \cdot [y_i = c]
$$
...
После этой правки STOLP дал сильное сжатие (9 прототипов вместо 120) **без потери точности на test**.
...
* `main.py`: pandas EDA, стандартизация, LOO-подбор $k$, оценка на test, сравнение с sklearn, отбор эталонов.
* `model.py`: numpy-only реализация расстояний, Parzen-KNN, LOO, CNN/STOLP, PCA-2D.
* `nn_utils.py`: вспомогательные функции.
