import numpy as np
from tqdm import tqdm
import random
from functools import cmp_to_key
from .optimizers import SGDOptimizer
from .losses import LogLoss


class SimpleSGDClassifier:
    def __init__(self, optimizer=SGDOptimizer(), loss=LogLoss(), penalty='l2', alpha=0.01, max_iter=100,
                 forget_rate=0.01, ordering='random', weight_init='random'):
        self.optimizer = optimizer
        self.penalty = penalty  # None | 'l2'
        self.alpha = alpha
        self.max_iter = max_iter
        self.loss = loss
        self.forget_rate = forget_rate
        self.weight_init = weight_init  # 'random' | 'corr' | 'multistart'
        self.ordering = ordering  # 'random' | 'large_margin_first'
        self.w = np.array([])
        self.Q = 0

    def fit(self, X, y, loss_callback):
        X = np.hstack([X, np.ones((X.shape[0], 1))])
        match self.weight_init:
            case 'random':
                self.w = np.random.normal(0, 2 / X.shape[1], X.shape[1])
            case 'corr':
                self.w = (X.T @ y) / np.sum(X ** 2, axis=0)
            case 'multistart':
                models = [SimpleSGDClassifier(max_iter=1) for _ in range(10)]
                best_params = models[0].w
                best_Q = float('inf')
                for model in models:
                    model.fit(X, y, None)
                    if model.Q < best_Q:
                        best_params = model.w
                        best_Q = model.Q
                self.w = best_params

        _xy = list(zip(X, y))

        random.shuffle(_xy)
        self.Q = 0
        for x, y in _xy:
            self.Q += self.loss.loss(x @ self.w) / len(_xy)

        for epoch in tqdm(range(self.max_iter)):

            match self.ordering:
                case 'random':
                    random.shuffle(_xy)
                case 'large_margin_first':
                    def compare(a, b):
                        return (b[0] @ self.w) * y - (a[0] @ self.w) * y

                    _xy = sorted(_xy, key=cmp_to_key(compare))

            for x, y in _xy:

                match self.penalty:
                    case 'l2':
                        self.w = self.optimizer.step(self.w * (1 - self.optimizer.lr * self.alpha),
                                                     lambda w: self.loss.derivative(
                                                         (x @ w) * y) * x * y + self.alpha * w)
                    case None:
                        self.w = self.optimizer.step(self.w, lambda w: self.loss.derivative((x @ w) * y) * x * y)
                self.Q = self.forget_rate * self.loss.loss((x @ self.w) * y) + (1 - self.forget_rate) * self.Q;

                if loss_callback:
                    loss_callback(self.Q)

    def predict(self, X):
        X = np.hstack([X, np.ones((X.shape[0], 1))])

        def sign(x):
            return x / np.abs(x)

        return sign(X @ self.w)
