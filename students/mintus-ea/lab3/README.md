...
Здесь $K(x_i, x_j)$ — функция ядра. Вектор весов восстанавливается как $w = \sum \alpha_i y_i x_i$ (только для линейного ядра), а смещение $b$ вычисляется по опорным векторам.
