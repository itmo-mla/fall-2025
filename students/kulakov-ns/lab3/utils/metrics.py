...
class MetricsEstimator:
    """Метрики качества бинарной классификации {-1, 1}."""
    ...
    def get_metrics(self, y_true: np.ndarray, y_pred: np.ndarray) -> "MetricsEstimator":
        """Вычислить все метрики и сохранить их в объекте."""
    ...
    def get_precision(y_true: np.ndarray, y_pred: np.ndarray) -> float:
        """Предполагаем метки {-1, 1} и 1 — позитивный класс."""
...
def evaluate(name: str, y_true: np.ndarray, y_pred: np.ndarray) -> None:
    m = MetricsEstimator().get_metrics(y_true, y_pred)
    print(f"\n{name}\n{'-' * len(name)}\n{m}")
