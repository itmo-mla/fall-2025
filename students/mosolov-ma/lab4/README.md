...
Реализация включает:
- Функцию `custom_pca` для выполнения PCA с возможностью указания количества компонент
