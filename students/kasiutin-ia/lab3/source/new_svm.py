...
class SVM:
    ...
    def _get_lagrangian(lambdas: np.ndarray, X: np.ndarray, y: np.ndarray, kernel: Kernel) -> float:
        # print("Res", - (lambdas.sum() - 0.5 * np.sum(np.outer(lambdas * y, lambdas * y) * kernel(X, X))))
    ...
    def _get_jac(lambdas: np.ndarray, X: np.ndarray, y: np.ndarray, kernel: Kernel) -> float:
        # print("grad", - (np.ones_like(lambdas) - lambdas * y * np.sum(lambdas * y * kernel_res, axis=1)))
        #  return - lambdas * (1 - y * np.sum(lambdas * y * kernel_res, axis=1))
        # return - (np.ones_like(lambdas) - lambdas * y * np.sum(lambdas * y * kernel_res, axis=1))
        G = y[:, np.newaxis] * kernel(X, X) * y[:, np.newaxis].T
        return - (np.ones_like(lambdas) - np.dot(lambdas, G))

    def fit(self, X: np.ndarray, y: np.ndarray, maxiter: int = 1000, ftol: float = 1e-3, support_vector_threshold = 1e-4) -> np.ndarray:
