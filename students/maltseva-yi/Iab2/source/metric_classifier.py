...
def select_prototypes_ccv(X, y):
    # Реализация жадного отбора эталонов по CCV для 1NN
    n_samples = len(X)
    L = n_samples
    ...
    for i in range(n_samples):
        # Индексы соседей (без самого объекта)
        idxs = [indices[i][j] for j in range(1, n_samples)]
        neighbor_indices.append(idxs)
        neighbor_labels.append(y[idxs])
    
    def compute_compactness_profile(prototype_mask):
        proto_indices = np.where(prototype_mask)[0]
        proto_set = set(proto_indices)
        ...
        for i in range(L):
            # Находим соседей среди эталонов
            neighbors_in_proto = []
            for j, neighbor_idx in enumerate(neighbor_indices[i]):
                if neighbor_idx in proto_set:
                    neighbors_in_proto.append((j, neighbor_idx))
                if len(neighbors_in_proto) >= L-1:
                    break

            for m, (order, neighbor_idx) in enumerate(neighbors_in_proto):
                if neighbor_labels[i][order] != y[i]:
                    profile[m] += 1
        
        return profile / L

    def compute_ccv_from_profile(profile, k=1, L=L):
        if k >= L:
            return 1.0
        ...
        for m in range(1, k+1):
            ...
            if m > L-1:
                ...
                numerator = comb(L-1-m, ℓ-1, exact=True) if ℓ-1 >= 0 else 0
                denominator = comb(L-1, ℓ, exact=True)
                R_m = numerator / denominator if denominator > 0 else 0
            
            ccv_sum += Π_m * R_m
        
        return ccv_sum

    prototype_mask = np.ones(n_samples, dtype=bool)

    k = 1
    profile = compute_compactness_profile(prototype_mask)
    current_ccv = compute_ccv_from_profile(profile, k, L)
    print(f"Начальная CCV (k={k}): {current_ccv:.4f}")
    
    improved = True
    iteration = 0
    
    while improved and np.sum(prototype_mask) > len(set(y)):
        improved = False
        best_ccv = current_ccv
        best_to_remove = -1

        candidate_indices = np.where(prototype_mask)[0]
        
        for idx in candidate_indices:
            temp_mask = prototype_mask.copy()
            temp_mask[idx] = False

            remaining_classes = set(y[temp_mask])
            if len(remaining_classes) < len(set(y)):
                continue

            temp_profile = compute_compactness_profile(temp_mask)
            temp_ccv = compute_ccv_from_profile(temp_profile, k, L)
            
            if temp_ccv < best_ccv:
                best_ccv = temp_ccv
                best_to_remove = idx
        
        if best_to_remove >= 0:
            prototype_mask[best_to_remove] = False
            current_ccv = best_ccv
            improved = True
            iteration += 1
            
            remaining = np.sum(prototype_mask)
            print(f"Итерация {iteration}: удалён {best_to_remove}, осталось {remaining}, CCV: {current_ccv:.4f}")
            
            if remaining <= len(set(y)) * 2:
                break
        else:
            for idx in candidate_indices:
                temp_mask = prototype_mask.copy()
                ...
                if temp_ccv <= current_ccv + 0.001:  # Допуск
                    best_ccv = temp_ccv
                    best_to_remove = idx
                    break
            ...
            if best_to_remove >= 0:
                ...
                remaining = np.sum(prototype_mask)
                print(f"Итерация {iteration} (с допуском): удалён {best_to_remove}, осталось {remaining}, CCV: {current_ccv:.4f}")
    
    proto_indices = np.where(prototype_mask)[0]
    print(f"\nВыбрано {len(proto_indices)} эталонов из {n_samples}")
    print(f"Финальная CCV: {current_ccv:.4f}")
    
    return X[proto_indices], y[proto_indices]
...
def main():
    ...
    X_proto, y_proto = select_prototypes_ccv(X_train, y_train)
